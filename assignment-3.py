...
def PlotScatterGraph(x, y, df, clusters):
    ...
    plt.ylabel(y)
    plt.title(
        f"CO2 emissions (metric tons per capita) For {clusters} clusters")
    plt.show()
...
def TransposeDataFrame(df):
    ...
    # clean the transposed dataframe those are completely empty 
    # and then drop column with missing entries
...
df_co2 = pd.read_csv(
    'API_EN.ATM.CO2E.PC_DS2_en_csv_v2_5455265.csv', skiprows=4)
...
# scatter plot with colours selected using the cluster numbers
plt.scatter(df_co2_t["United States"],
            df_co2_t["Brunei Darussalam"], c=labels, cmap="tab10")
...
plt.figure()
plt.plot(df_pop_t.index, df_pop_t["United States"],
